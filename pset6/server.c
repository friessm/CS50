...
char* indexes(const char* path)
{
    ...
    // store copy of path in buffer 
    char buffer[strlen(path)];
    strcpy(buffer, path);
    
    // check if the file is in the current directory
    // initialise index.php & index.html
    char* i_php = "index.php\0";
    char* i_html = "index.html\0";
    
    // check if path is ending with /
    for (int i = 0; buffer[i] != '\0'; i++)
    {
        // if path ends with /, everything is fine, hence break
        if (buffer[i] == '/' && buffer[i + 1] == '\0')
        {
            break;
        }
        // if not ending with /, concatenate / and indexes
        if (buffer[i] != '/' && buffer[i + 1] == '\0')
        {
            strcat("/", i_php);
            strcat("/", i_html);
    ...
    // is strlen(i_php) giving the right amount of length afer the concatenation?
    // php
    char* php = malloc(strlen(path) + strlen(i_php)) + 1;
    strcpy(php, path);
    strcat(php, i_php);
    
    // html
    char* html = malloc(strlen(path) + strlen(i_html) + 1);
    strcpy(html, path);
    strcat(html, i_html);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    // initialise variable to track size of the data
    *length = 0;
    ...
    *content = NULL;
    ...
    // error check if file is actually open
    if (file == NULL)
    {
        return false;
    }
    
    // fread returns the number of objects read (here: sizeof(char))
    for (int c = fread(buffer, 1, 512, file); c != 0; c = fread(buffer, 1, 512, file))
    {
        // continuously update pointer because realloc might copy data to another part of the memory
...
const char* lookup(const char* path)
{
    ...
    // seperate and store file extension of path in a variable
    char* extension = strchr(path, '.');
    
    if (extension != NULL)
    {
        // compare the extension with the supoorted list
        // css
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // copy string from line to buffer
    strcpy(buffer, line);
    ...
    // ensure request-target does not contain a "
    ...
    {
        if(tmp[i] != http_v[i]) correct = false;
    }
    ...
    // check for two SPs before HTTP-Version
    ...
    // declare i here so it can be accessed outside of the for-loop
    int i;
    
    // find ? and store absolute path in abs_path, end when reaching SP
    for (i = 0; target[i] != ' '; i++)
    {
        // if ? is reached, end string with null-terminator and break fro-loop
        if (target[i] == '?')
        {
            abs_path[i] = '\0';
            break;
        ...
        // store path string in abs_path
        abs_path[i] = target[i];
    }
    
    // ? and query string in the request-target
    if (target[i] == '?' && target[i + 1] != '\0')
    {
        i++; // used to get to the first char of the query
        for (int y = 0; target[i] != '\0'; i++, y++)
        {
            query[y] = target[i];
            
            // check for end of request-target string
            if (target[i + 1] == '\0')
            {
                query[y + 1] = '\0';
            }
    ...
    // only ? and no query string present
    else if (target[i] == '?' && target[i + 1] == '\0')
    {
        query[0] = '\0';
    }
    
    // no ? present in target string
    else if (target[i] != '?')
    {
        query[0] = '\0';
    }
